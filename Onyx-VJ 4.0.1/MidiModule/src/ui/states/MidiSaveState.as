...
package ui.states {
	...
	public final class MidiSaveState extends ApplicationState {
			...
			if (event.type === Event.SELECT) {
				...
				for (i in Midi.controlsSet) {
					if (i!='null') {
						...
						//ctrlXML.appendChild(control.name);
						log("control" + control.getParameter().value); 
						/*..getParameter().getMetaData(Midi.tag));
		...
		{
		    var file:File = File.applicationStorageDirectory.resolvePath( nowDate +  "midiSaveState.log" );
		    var fileMode:String = ( clear ? FileMode.WRITE : FileMode.APPEND );
		
		    var fileStream:FileStream = new FileStream();
		    fileStream.open( file, fileMode );
		
		    fileStream.writeUTFBytes( "MidiSaveState: " + text + "\n" );
		    fileStream.close();
