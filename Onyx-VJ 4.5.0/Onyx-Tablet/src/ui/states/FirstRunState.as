...
package ui.states {
	...
	public final class FirstRunState extends ApplicationState {
		...
		{
			Console.output('onZoom:',event.scaleX);
			Console.output('onZoom,width:',DISPLAY_STAGE.width);
			Console.output('onZoom,height:',DISPLAY_STAGE.height);
			Console.output('onZoom,stageWidth:',DISPLAY_STAGE.stageWidth);
			Console.output('onZoom,stageHeight:',DISPLAY_STAGE.stageHeight);
			Console.output('onZoom,numChildren:',DISPLAY_STAGE.numChildren);
			/*DISPLAY_STAGE.width = 100;
			DISPLAY_STAGE.height = 200;*/
			const brw:Browser = WindowRegistration.getWindow('FILE BROWSER') as Browser;
			if (brw) {
				brw.x=100;
			}
			const layers:Layer = WindowRegistration.getWindow('LAYERS') as Layer;
			if (layers) {
				
				for each (var layer:LayerImplementor in layers) {
					layer.display.y = 65;
				}
			}
		...
		{
			Console.output('ReOrient,width:',DISPLAY_STAGE.width);//1272
			Console.output('ReOrient,height:',DISPLAY_STAGE.height);//808
			Console.output('ReOrient,stageWidth:',DISPLAY_STAGE.stageWidth);// 1280 or 800
			Console.output('ReOrient,stageHeight:',DISPLAY_STAGE.stageHeight);//752 or 1232
		}
