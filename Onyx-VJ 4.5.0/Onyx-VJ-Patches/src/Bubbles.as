...
// forked from Nyarineko's è¡çªåˆ¤å®šã®å­¦ç¿’
/*ã‚ªãƒ©ã‚¤ãƒªãƒ¼ã€€è¡çªåˆ¤å®šã®å­¦ç¿’*/
package
{
	...
	public class Bubbles extends Patch
	{
		...
		private function makeBalls():void
		{
			...
			for (var i:int = 0; i < _numBalls; i++)
			{
				...
				ball.x = Math.random() * DISPLAY_WIDTH;
				ball.y = Math.random() * DISPLAY_HEIGHT;
		...
		private function updateBalls():void
		{
			...
			for (var i:int = 0; i < _numBalls; i++)
			{
				...
				// ãƒœãƒ¼ãƒ«ã‚’ç§»å‹•ã—ã€å£ã«è·³ã­è¿”ã‚‰ã›ã‚‹
				...
				else if (ball.x > DISPLAY_WIDTH )
				{
					ball.x = DISPLAY_WIDTH;
					ball.vx *= -1;
				}
				...
				else if (ball.y > DISPLAY_HEIGHT )
				{
					ball.y = DISPLAY_HEIGHT;
					ball.vy *= -1;
				}
		...
		private function checkCollision(ballA:Ball, ballB:Ball):void
		{
			...
			// 2ã¤ã®ãƒœãƒ¼ãƒ«ã®è¡çªåˆ¤å®šã‚’è¡Œã†
