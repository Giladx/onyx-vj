...
{    
	...
	public class ElectroCardiogram extends Patch
	{
		private var PV:PVElectroCardiogram;
		...
		{
			Console.output('ElectroCardiogram v 0.0.9');
			Console.output('Created by Bruce LANE (http://www.batchass.fr)');
			PV = new PVElectroCardiogram();
...
class PVElectroCardiogram extends BasicView
{
	...
	public function PVElectroCardiogram() 
	{
		...
		bmpd = new AssetForElectroCardiogram();
	...
	private function loop(evt:TimerEvent=null):void 
	{
		for(var xr:int = 0; xr < _imageWidth ; xr+= scanStep) 
		{
			var color:Number = 0xFFFFFF;
			//if ( yr == 0 ) color = 0xFFFFFF else color = bmpd.getPixel(xr, yr);
		...
		{
			timer.stop();
			Console.output("lines:" + lines.lines.length);
		}
	}	
