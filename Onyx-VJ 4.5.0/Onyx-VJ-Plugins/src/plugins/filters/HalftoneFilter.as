...
package plugins.filters 
{
	...
	public final class HalftoneFilter extends Filter implements IBitmapFilter 
	{
		private var matrix_:Matrix;
		private var smooth:Smooth;  
		private var tone:HalftoneColor; 
		private var _amount:int = 20;
		private var _temp:BitmapData = createDefaultBitmap();
		/**
		 * 	@constructor
		 */
		public function HalftoneFilter():void 
		{
			
			parameters.addParameters(
				new ParameterInteger('amount', 'amount', 10, 50, 20),
		...
		{
			source.draw(_temp, matrix_);
			
			smooth.applyEffect(source);   
			tone.applyEffect(source);  
		}		
