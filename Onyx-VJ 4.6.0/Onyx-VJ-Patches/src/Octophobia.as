...
package  {
	...
	public class Octophobia extends Patch {
		...
		public function Octophobia() {
			...
			sprite = new Sprite();
			sprite.addChild(vpl = new VolumetricPointLight(DISPLAY_WIDTH, DISPLAY_HEIGHT, new OctopusGenerator(465, 465), [0x687072, 0x121108, 0], [0.88, 0.22, 0.77], [8, 16, 244]));
			vpl.startRendering();
			vpl.scale = 4;
			vpl.opaqueBackground = 0x000000;
...
class OctopusGenerator extends Bitmap {
	private const OFFSET : Number = 96;
	private const CURVE_QUANT : uint = 128;
	private const CURVE_LENGTH : uint = 12;
	private const N_AMP : Number = 8;
	private const N_AMP2 : Number = N_AMP * 24;
	private const TANGENT_OFFSET : Number = 1.5;
	private const TIME_SCALE : Number = 128;
	...
	private var _fadeCT : ColorTransform = new ColorTransform(1, 1, 1, 0.72);
	...
	public function OctopusGenerator(w : uint, h : uint) : void {
		...
		while (nn < CURVE_QUANT) {
			...
			while (n < CURVE_LENGTH) {
				...
				_controlPoints[nn].push(new Vector3D(o.x - N_AMP + N_AMP2 * Math.random(), o.y - N_AMP + N_AMP2 * Math.random(), o.z - N_AMP + N_AMP2 * Math.random(), o.w));
				++n;
			}
			++nn;
		}
