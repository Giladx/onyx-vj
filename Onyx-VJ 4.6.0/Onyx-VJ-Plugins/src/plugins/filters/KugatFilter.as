/**
 * Copyright shivaazari ( http://wonderfl.net/user/shivaazari )
 * MIT License ( http://www.opensource.org/licenses/mit-license.php )
 * Downloaded from: http://wonderfl.net/c/3VE6
 */
...
package plugins.filters {
	...
	public final class KugatFilter extends Filter implements IBitmapFilter {
		...
		public function KugatFilter():void {
...
class Dot extends Sprite {
	...
	public function draw(rgb:ColorRGB):void {
		...
		var g:Graphics = circle.graphics;
			g.clear();
			g.beginFill( 0xFF << 8 * (2 - i), 0.75 );
			g.lineStyle(0, 0x0, 0.75);
			g.drawCircle(Math.random() - 0.5, Math.random() - 0.5, radius_ * v[i] / 255);
			g.endFill();

 
			/*
	 var canvas:Graphics = circle.graphics;
			canvas.clear();
			canvas.beginFill(v[i], 0.25);
			canvas.lineStyle(0, v[i], 0.75);
			canvas.drawRect(0.5, 0.5, radius_, radius_);
			canvas.moveTo(0.5, 0.5);
			canvas.lineTo(0.5 + radius_, 0.5 + radius_);
			canvas.moveTo(0.5 + radius_, 0.5);
			canvas.lineTo(0.5, 0.5 + radius_);
			canvas.endFill();
*/
